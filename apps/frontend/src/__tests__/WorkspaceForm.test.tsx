...
function renderForm(onSuccess = vi.fn()) {
  ...
  return render(
    createElement(QueryClientProvider, { client: queryClient },
      createElement(WorkspaceForm, { onSuccess })
    )
  )
}
...
describe('WorkspaceForm', () => {
  ...
  it('calls api.createWorkspace with trimmed name on submit', async () => {
    const mockWorkspace = { workspaceId: 'ws-1', name: 'My Team', status: 'pending_connection' as const, licenseJwt: 'jwt' }
  ...
  it('shows setup-submitting testid while pending', async () => {
    let resolve!: (v: Awaited<ReturnType<typeof api.createWorkspace>>) => void
